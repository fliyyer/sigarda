  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (rememberMe) {
        ...
        setTokenSession(response.authToken);
      }
      if (user.role === "monev") {
        navigate("/");
      } else if (user.role === "spt") {
        navigate("/spt");
      } else {
        navigate("/pelayanan");
      }
      ...
      <div className="w-1/2 bg-darker flex items-center justify-center">
        <form className="bg-[#fff] p-12 rounded-[40px]">
          ...
          <div className="mb-4">
            <label
              htmlFor="username"
              className="text-[#333333] text-[15px] leading-[20px] tracking-[0.3px] font-normal mb-2"
            >
              Email
            </label>
            <input
              ...
              className="w-full border bg-[#4D4D4D] bg-opacity-10  p-2 placeholder:text-[#808080] placeholder:text-[14px] rounded-lg"
            />
          </div>
          <div className="mb-4">
            <label
          ...
          <div className="mb-4 flex justify-between">
            ...
            <Link
              to="/forgot-password"
              className="text-sm hover:underline text-main-color"
            >
              Lupa Password?
            </Link>
          </div>
          ...
          <p className="text-center mt-6 text-sm">
            Belum Punya Akun?{" "}
            <Link to="/register" className="text-main-color hover:underline">
              Kontak Administrator
            </Link>
          </p>
          <footer className="flex justify-end text-xs mt-16 2xl:mt-44 text-[#666666]">
            © AT 2024
          </footer>
        </form>
      </div>
